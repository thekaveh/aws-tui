"""Dict-backed FileSystemProvider used by domain and VM-layer tests.

Kept inside ``tests/`` rather than the production tree so it doesn't ship
in the wheel. It conforms to :class:`aws_tui.domain.filesystem.FileSystemProvider`
and is the primary fake the cross-FS / VM / integration tests reuse.
"""
