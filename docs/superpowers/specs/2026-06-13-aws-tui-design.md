...
### 2.2. Non-goals (explicit, so they don't sneak in)
...
### 5.2. Keymap (macOS-tailored, customizable)
...
#### 5.5.1. Carbon (default) — near-monochrome with ice-blue accent
...
#### 5.5.2. Voidline — neon cyan + magenta on near-black
...
#### 5.5.3. Lattice — mint-teal + lavender on deep teal
...
#### 5.5.4. Amber CRT — retro phosphor monitor
...
## 6. MVVM wiring with VMx

### 6.1. VM tree with primitives
...
### 6.2. Commands (`RelayCommand` with reactive `canExecute`)
...
### 6.8. View ↔ VM binding (Textual side)
...
### 7.3. Auto-discovery + SSO cache probe
...
#### 7.4.1. Flow 1 — Cold start, default connection, valid SSO token
...
#### 7.4.2. Flow 2 — Switch connection (AWS → MinIO)
...
#### 7.4.3. Flow 3 — SSO login (first connect or expired)
...
#### 7.4.4. Flow 4 — SSO token expires mid-session
...
#### 7.4.5. Flow 5 — First run, no connections configured
...
### 8.5. Per-transfer state machine
...
### 8.6. Interrupted-transfer diagnostic journal
...
### 9.2. Unit tier — fake providers, not mocks
...
### 9.3. Integration tier — real-ish backends
...
| S3-compatible test service | `testcontainers-python` with `adobe/s3mock:5.1.0@sha256:65cf60155a2e235fe7d5bf6c633747d6fc7ed93f9f5a6727d86470026b83c2a2` | `S3FS` against an independent implementation; catches path-style and wire-protocol quirks without a vulnerable or license-gated MinIO image |
...
### 9.4. Snapshot tier — Textual views
...
### 9.7. aws-tui-specific lifecycle suite
...
## 10. Distribution, CI release-side, repo bootstrap, milestones
...
### 10.5. CI workflows (release-side; test matrix in §8.9)
...
### 10.6. Repo bootstrap — first commits after spec sign-off
...
### 10.7. Milestones (solo dev, calendar-time, ~50% buffer)
