...
## 2. Background / implementation-time baseline
...
### 3.1. `ActionRegistry` is the set of actually-handled actions
...
### 3.2. Resolver materializes only handled actions
...
### 3.3. Single dispatch entry point
...
### 3.5. Align `KeymapStore.DEFAULT_BINDINGS` to reproduce today's behavior
...
### 3.6. Install resolver bindings at runtime
