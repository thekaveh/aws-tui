...
## 1. How to use this spec — reading guide for the next worker
...
### 2.1. The recurring bug pattern that motivated this work
...
### 2.2. The conversation that surfaced the design
...
### 2.3. Mistakes made during the review session — record so the next worker
...
## 3. What VMx 2.6.1 actually provides
...
### 3.1. `CompositeVM[VM]` (and `CompositeVMOf[M, VM]`)
...
### 3.4. `FormVM[TM]` (and `FormVMBuilder`)
...
### 3.5. `IDialogService` / `DialogService` / `NullDialogService`
...
### 3.6. `HierarchicalVM[TModel, TVM]` (and `HierarchicalVMBuilder`)
...
### 3.7. `GroupVM` (and `GroupVMBuilder`)
...
### 3.8. `ServicedObservableCollection`
...
### 3.9. `ForwardingComponentVM` / `ForwardingCompositeVM`
...
### 3.10. `Batch` and `ObservableDictionary`
...
## 4. Current state in aws-tui

### 4.1. Inventory: what we currently hand-roll
...
### 4.2. 3.2.bis. View-side state that should live in the VM layer
       — regardless of VMx
...
#### 5.2.1. `NavMenuVM` — finish the incomplete adoption
...
#### 5.2.2. `JobRunsVM` → `CompositeVM[JobRunVM]` (PagedComposition dropped)
...
#### 5.2.3. `PaneVM` — `CompositeVM` vs `HierarchicalVM` evaluation
...
#### 5.2.4. `ApplicationsVM` → `CompositeVM[ApplicationVM]`
...
#### 5.2.5. `TransfersVM` → `ServicedObservableCollection[TransferVM]`
...
#### 5.2.6. `ToastStackVM` → `CompositeVM[ToastVM]`
...
#### 5.2.7. `S3ConnectionsVM` → `FormVM<S3Connection>`
...
#### 5.2.8. Modal VMs → custom aws-tui VMs composing VMx (all four)
...
#### 5.2.9. `CommandPaletteVM` + `ThemePickerVM`
...
### 5.3. The FocusCoordinatorVM — promoted into this spec
...
### 6.1. Ordering decision — does the focus / selection refactor come BEFORE
...
### 6.3. Phase 1 — leaf-VM migrations + finish NavMenu adoption
...
### 6.4. Phase 2 — pagination + the EMR runs train
...
### 6.8. Phase 6 — Command palette + theme picker
...
### 6.9. Phase 7 — `FocusCoordinatorVM`
...
### 7.4. Regression anchors from prior bug train
...
### 8.1. `PagedComposition` may not fit token-pagination — CLOSED
...
### 8.3. Lifecycle propagation surprises
...
### 8.4. Adoption resistance — the "abstraction tax"
...
### 9.2. Domain layer changes

Pure adoption of an existing framework primitive in the VM layer.
Domain stays as-is.

### 9.3. ~~`FocusCoordinatorVM`~~ — was here, promoted into the spec at
       review round 3
...
## 11. 9.bis. Resolutions from the brainstorm session — 2026-06-28
...
### 11.1. 9.bis.1. Q7 / §4.2.2 — PaneVM: choose Option A (`CompositeVM[EntryVM]`)
...
### 11.2. 9.bis.2. Q1 / §4.2.1 — JobRunsVM: drop `PagedComposition`
...
### 11.3. 9.bis.3. Q2 / §7.2 — Filter coupling: Option C (derived view as a VM @property)
...
### 11.4. 9.bis.4. Q3 — `auto_construct_on_add=True` composes with `ToastVM`
...
### 11.5. 9.bis.5. Q4 — `FormVM` cross-field validators: fits via custom predicate
...
### 11.6. 9.bis.6. Q5 / §4.2.7 — `IDialogService` for the four modal VMs: 2 adopt, 2 stay
...
### 11.7. 9.bis.7. Q6 — `ServicedObservableCollection`: premise was wrong; mistake 9 recorded
...
### 11.9. 9.bis.9. MVVM-half re-classification of today's bug train
...
### 11.11. 9.bis.11. Round 3 — directive "compose, don't reject" applied (2026-06-29)
...
#### 11.11.1. §9.bis.2 (Q1 JobRunsVM) — re-framed
...
#### 11.11.2. §9.bis.3 (Q2 filter coupling) — re-framed
...
#### 11.11.3. §9.bis.5 (Q4 `FormVM` cross-field validators) — re-framed
...
#### 11.11.4. §9.bis.6 (Q5 modal VMs) — re-framed (significant: no VM stays hand-rolled)
...
#### 11.11.5. Other resolutions — unchanged under the directive
...
### 11.13. 9.bis.13. Upstream feedback artifact
...
### 14.5. A.5. Repo state at spec time
...
## 15. Appendix B — Per-VM cross-reference table
...
## 16. Appendix C — VMx primitive cheat sheet (for quick lookup during brainstorm)
