...
- Run and Stop use exact `5x3` outer terminal geometry in every command state.
...
- [x] **Step 1: Write failing SQL and handoff assertions**
...
- [x] **Step 2: Run the focused tests and observe old three-part SQL failures**
...
- [x] **Step 4: Re-run focused SQL and handoff tests**

Expected: all selected tests pass and no execution occurs during handoff.

- [x] **Step 5: Commit the working slice**
...
- [x] **Step 1: Write failing domain classification and redaction tests**
...
- [x] **Step 2: Run focused domain tests and observe missing types**
...
- [x] **Step 3: Add narrow start-query classification**
...
- [x] **Step 4: Preserve categories through runner and VM mapping**
...
- [x] **Step 5: Run domain, runner, and query-VM tests**
...
- [x] **Step 6: Commit the error slice**
...
**Interfaces:**
- Produces: exact `5x3` `#athena-execute` and `#athena-cancel` regions with centered glyph content.

- [x] **Step 1: Tighten the rendered-geometry test**

Assert for both buttons:

```python
assert button.region.size == Size(5, 3)
assert button.styles.min_width == Scalar.from_number(5)
assert button.styles.max_width == Scalar.from_number(5)
...
- [x] **Step 2: Run the focused UI test and observe the `3x3` failure**
...
- [x] **Step 3: Lock both dimensions in component CSS**

Set `width`, `min-width`, and `max-width` to `5`; set `height`, `min-height`, and `max-height` to `3`; retain centered compact flat buttons and the existing margin.

- [x] **Step 4: Run focused UI and snapshot tests**
...
- [x] **Step 1: Synchronize user and architecture contracts**

Document context-relative generated SQL, exact request-context ownership,
enforced-workgroup output behavior, safe rejection categories, and `5x3`
control geometry. Mark the design implemented only after verification succeeds.

- [x] **Step 2: Run formatting, lint, typing, and focused verification**
...
- [x] **Step 3: Run complete tests and documentation verification**
