...
## 3. Task 1: Add `unreachable_connections` field to `AppContext`
...
## 4. Task 2: `action_swap_source` filters the ring + emits skip toast
...
## 5. Task 3: Auto-observe `PaneState.UNREACHABLE` transitions on the active panes
...
## 6. Task 4: CHANGELOG + final verification
...
## 7. Acceptance check (re-stated from spec §6)
