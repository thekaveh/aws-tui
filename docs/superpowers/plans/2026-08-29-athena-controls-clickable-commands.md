...
- [x] Add assertions that both controls have `content_region.height >= 1`, remain inside the controls content area, and precede the editor.
- [x] Run the focused test and confirm it fails with content height `0`.
...
- [x] Add a failing acceptance test that revisits a mounted Athena page and requires explicit visible projection after Glue `Q`.
- [x] Run the focused integration test and confirm the missing transaction-level projection fails.
...
- [x] Add failing tests that click an enabled chip, observe one registry dispatch, click a disabled chip, observe none, and assert chips remain non-focusable.
- [x] Run the focused tests and confirm enabled clicks do not yet dispatch.
...
- [x] Run focused Athena, Glue navigation, legend, binding, and action-registry test modules.
- [x] Regenerate affected snapshots and inspect wide, compact, and narrow output.
- [x] Run `uv run ruff check .`, `uv run mypy src`, and the complete pytest suite.
- [x] Review `git diff --check`, changed-file scope, and documentation consistency.
