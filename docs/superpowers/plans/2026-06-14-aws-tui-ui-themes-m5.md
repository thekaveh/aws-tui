...
## 1. Task 1: `ui/actions.py` + `ui/bindings.py` — action/binding registry
...
## 2. Task 2: `ui/themes/*.tcss` — full theme files
...
## 3. Task 3: `ui/widgets/` — chrome widgets (status bar, hint legend, toast, services menu)
...
## 4. Task 4: `ui/widgets/pane.py` + `ui/widgets/dual_pane.py`
...
## 5. Task 5: overlay widgets — command palette, confirm modal, quick look, transfers tray
...
## 6. Task 6: snapshot tests per theme
...
## 7. Task 7: app composition — replace M0 hello-world
...
## 8. Task 8: commit per task + push + tag v0.6.0
...
## 9. Layer-rule decision (implementation-time revision)
