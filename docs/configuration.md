...
| `TEXTUAL_SMOOTH_SCROLL` | `0` — aws-tui sets it at launch unless you already have | Textual's switch for the in-band window-resize protocol (DEC private mode 2048). aws-tui defaults it off so the `SIGWINCH` resize fallback stays live; set it to `1` for Textual's own default. Despite the name it gates no scrolling in Textual 8.2.8. See [Window resize](platforms.md#31-window-resize). |
